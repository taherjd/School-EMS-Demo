...
export async function createStudent(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  if (createPortal && guardian.email) {
    ...
    if (existing) portalUserId = existing.id;
      ...
      const tempPassword = `Parent-${Math.random().toString(36).slice(2, 8)}!`;
      const u = await prisma.user.create({ data: { email: guardian.email.toLowerCase(), passwordHash: await hashPassword(tempPassword), name: `${guardian.firstName} ${guardian.lastName}`, role: "PARENT" } });
      portalUserId = u.id;
      await audit(session.userId, "CREATE_PORTAL_USER", "User", u.id, { tempPasswordIssued: true });
      console.info(`Parent portal account created for ${guardian.email} with temporary password ${tempPassword}`);
    }
  }
